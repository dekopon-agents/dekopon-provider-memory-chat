...
for pair in \
  'dekopon-provider-sdk 0.11.1' \
  'dekopon-provider-storage 0.11.1' \
  'serde 1.0.229' \
  'serde_json 1.0.151' \
  'wit-bindgen 0.44.0'; do
...
cargo +1.97.0 tree --locked --target wasm32-unknown-unknown --edges normal,build >"$graph"
grep -Fq 'dekopon-provider-storage v0.11.1' "$graph"
