...
for required in '"v0.1.0"' 'git cat-file -t' 'git merge-base --is-ancestor' 'subject-path: dist/memory-chat-provider.wasm' 'predicate-type: https://cyclonedx.org/bom' 'memory-chat-provider.wasm:application/wasm' 'application/vnd.dekopon.provider.v1+wasm' 'provider-memory-chat/versions' 'draft: false' 'make_latest: "false"' 'cleanup_failed_release:'; do grep -Fq "$required" "$release"; done
